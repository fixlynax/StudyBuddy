...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<a href='#' onclick='confirmDelete(\"{$row['resourceID']}\")' class='delete-button'><i class='fas fa-trash-alt'></i></a>";
        echo "</td>";
        echo "</tr>";
    }
} else {
...
function confirmDelete(resourceID) {
    if (confirm('Are you sure you want to delete this resource?')) {
        window.location.href = 'deleteResourceAdmin.php?id=' + resourceID;
    }
}
</script>